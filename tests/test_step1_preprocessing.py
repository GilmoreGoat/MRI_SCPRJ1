...
def test_build_graph_threshold_above_one():
    ...
    assert G.number_of_nodes() == num_rois
    assert G.number_of_edges() == 0

def test_build_graph_threshold_at_one():
...
def test_build_graph_threshold_negative():
...
def test_build_graph_perfect_correlation():
    ...
    for u, v, d in G.edges(data=True):
        assert pytest.approx(d['weight']) == 1.0

def test_build_graph_zero_correlation():
